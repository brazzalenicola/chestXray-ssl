...
def get_nih_dataloaders(
    ...
    n_shards = len([p for p in os.listdir(shard_root_path) if 'nih' in p])
    ...
    train_shards = os.path.join(
        shard_root_path, f"nih14-data-{{{train_start}..{train_end}}}.tar")
    val_shard = os.path.join(
        shard_root_path, f"nih14-data-{{{val_start}..{val_end}}}.tar")      
    ...
    nih_trainloader = torch.utils.data.DataLoader(
        train_dataset, 
        batch_size=batch_size,
        num_workers=num_workers
    )
    nih_validloader = torch.utils.data.DataLoader(
        val_dataset, 
        batch_size=batch_size,
        num_workers=num_workers
    )

    return nih_trainloader, nih_validloader
