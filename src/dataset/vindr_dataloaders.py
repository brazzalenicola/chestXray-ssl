...
def get_vindr_dataloaders(
    ...
    train_end = str(n_shards-1).zfill(6)

    # ref: https://stackoverflow.com/a/42521252
    train_shards = os.path.join(
        shard_root_path, f"vindr_cxr-train-{{{train_start}..{train_end}}}.tar")
    val_shard = os.path.join(
        shard_root_path, f"vindr_cxr-dev-{str(0).zfill(6)}.tar")      
    ...
    vindr_trainloader = torch.utils.data.DataLoader(
        train_dataset, 
        batch_size=batch_size,
        num_workers=num_workers
    )
    vindr_validloader = torch.utils.data.DataLoader(
        val_dataset, 
        batch_size=batch_size,
        num_workers=num_workers
    )

    return vindr_trainloader, vindr_validloader
