...
def add_subfolder(row):
   split = row.split("/")
   split.insert(-1, "mod-rx")
   return "/".join(split)
...
def write_bimcv_pos_shards(out_pattern, df, maxcount=1000, maxsize=(10e9)):
    with wds.ShardWriter(out_pattern, maxcount=maxcount, maxsize=maxsize) as sink:
        for idx in range(df.shape[0]):
            ...
            image = readfile(os.path.join("/scratch/cs/chest-xray-ssl/BIMCV-COVID19-cIter_1_2-Negative/covid19_neg", img_path))
...
if __name__ == "__main__":
    ...
    bimcv_path = "/scratch/cs/chest-xray-ssl/BIMCV-COVID19-cIter_1_2-Negative" 

    df = pd.read_csv(os.path.join(bimcv_path, 'partitions_neg.csv'), sep=";", usecols=["subject", "session", "filepath"])
    ...
    df["path"] = df["path"].apply(lambda row: add_subfolder(row))
     
    df_labels = pd.read_csv(os.path.join(bimcv_path, 'labels_covid_neg.csv'), sep=";")
    ...
    write_bimcv_pos_shards(
        "/scratch/cs/chest-xray-ssl/sharded_datasets/bimcv_neg/bimcv_neg-data-%06d.tar", 
